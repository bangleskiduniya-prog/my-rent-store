  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="space-y-4">
        {orders.map((order) => (
          <div key={order.id} className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-6 items-center">
            <div>
            ...
            <div>
              ...
              <p className="text-xs text-[#bcac76] font-bold">Rs. {order.price} x {order.quantity}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Address</p>
              <p className="text-xs text-gray-500 leading-relaxed"><MapPin className="w-3 h-3 inline mr-1" /> {order.customer?.address}</p>
            </div>
            <div className="text-right">
               <span className="bg-green-50 text-green-600 px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest">New Order</span>
            </div>
          </div>
        ))}
        {orders.length === 0 && <p className="text-center py-20 text-gray-400 italic">No orders received yet.</p>}
      </div>
    </div>
  );
}